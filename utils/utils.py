...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for image_i, pred in enumerate(prediction):
        ...
        if batched:
            ...
            if method == 'v_batch': # torchvision NMS
                i = torchvision.ops.boxes.batched_nms(boxes, scores, c, iou_thres)
                
            elif method == 'f_batch':  # FastNMS from https://github.com/dbolya/yolact
                boxes += c.view(-1, 1) * max_wh
                iou = box_iou(boxes, boxes).triu_(diagonal=1)  # upper triangular iou matrix
                i = iou.max(dim=0)[0] < iou_thres

            elif method == 'cluster_nms':  # 
                boxes += c.view(-1, 1) * max_wh
                iou = (box_iou(boxes, boxes).triu_(diagonal=1)) >=iou_thres  # upper triangular iou matrix
                B = iou
                for i in range(999):
                    A=B
                    maxA=A.max(dim=0)[0]
            ...
            elif method == 'cluster_diounms':  # 
                boxes += c.view(-1, 1) * max_wh
                iou = (box_diou(boxes, boxes).triu_(diagonal=1)) >=iou_thres  # upper triangular iou matrix
                B = iou
                for i in range(999):
                    A=B
                    maxA=A.max(dim=0)[0]
            ...
            elif method == 'cluster_weighted_nms':  
                boxes += c.view(-1, 1) * max_wh
                iou = box_iou(boxes, boxes).triu_(diagonal=1)  # upper triangular iou matrix
                B = iou
                for i in range(200):
                ...
                weights = (B*(B>iou_thres).float() + torch.eye(n).cuda()) * (scores.reshape((1,n)))
                xx1 = pred[:,0].expand(n,n)
                yy1 = pred[:,1].expand(n,n)
                xx2 = pred[:,2].expand(n,n)
                yy2 = pred[:,3].expand(n,n)
                ...
                pred = torch.stack([xx1, yy1, xx2, yy2], 1)
                pred = torch.cat((pred,scores.unsqueeze(1),c.unsqueeze(1)),1)

            elif method == 'cluster_weighted_diounms':  
                boxes += c.view(-1, 1) * max_wh
                iou = box_diou(boxes, boxes).triu_(diagonal=1)  # upper triangular iou matrix
                B = iou
                for i in range(200):
                    A=B
                    maxA=A.max(dim=0)[0]
        ...
        for c in cls.unique():
            ...
            elif method == 'weighted_nms':  # weighted mixture box
                while len(dc):
                    if len(dc) == 1:
                        det_max.append(dc)
                        break
                    iou = bbox_iou(dc[0], dc)  # iou with other boxes
                    i = iou  > iou_thres
                    weights = ((dc[i, 4:5].squeeze(1))*iou[i]).unsqueeze(1)
                    dc[0, :4] = (weights * dc[i, :4]).sum(0) / weights.sum()
                    det_max.append(dc[:1])
                    dc = dc[i == 0]
